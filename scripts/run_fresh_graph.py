...
Usage:
  EIGEN_CORPUS_DSN=postgresql://…/eigen  python scripts/run_fresh_graph.py \
      --domain elevenlabs.io --name "ElevenLabs" --tenant fresh1                 # dry run
  EIGEN_CORPUS_DSN=…  python scripts/run_fresh_graph.py --domain … --name … --tenant fresh1 --live
"""
...
async def _run(a) -> int:
    ...
    if not dsn:
        print("set EIGEN_CORPUS_DSN", file=sys.stderr)
        return 2
