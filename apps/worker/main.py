...
never double-process (in prod, set EIGEN_INGEST_IN_API=false on the API so ingestion runs ONLY here).
...
def main() -> None:
    ...
    if not dsn:
        raise SystemExit("EIGEN_CORPUS_DSN is required for the ingest worker")
    ...
    from eigen_kernel.runtime.build import load_active_vertical
    vertical = load_active_vertical().name
    print(f"[worker] eigen INGEST WORKER — vertical={vertical} (docling PDF parsing enabled)", flush=True)
