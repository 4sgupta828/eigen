...
function CompCard({ p, cols }: { p: CompPlayer; cols: { key: string; label: string }[] }) {
  ...
  const val = (k: string) => <>{(cell(k)?.text || "").trim()} {src(k)}</>;
  ...
  return (
    <div className={`th-comp-card${p.is_subject ? " th-comp-card-you" : ""}`}>
      ...
      {empty ? <div className="th-cell-empty">Not found in the open record.</div> : (
        <>
          {has("central_idea") ? <div className="th-comp-lead">{val("central_idea")}</div> : null}
          {statKeys.length ? <div className="th-comp-stats">{statKeys.map((k) => (
            <div key={k} className="th-comp-stat" style={{ ["--dc" as string]: compColor(k) }}>
              ...
              <span className="th-comp-stat-v">{val(k)}</span>
            </div>
          ))}</div> : null}
          {narr.length ? <div className="th-comp-dims">{narr.map((k) => (
            <div key={k} className="th-comp-dim" style={{ ["--dc" as string]: compColor(k) }}>
              ...
              <span className="th-comp-dv">{val(k)}</span>
            </div>
          ))}</div> : null}
        </>
      )}
...
function CompTab({ comp, id, owner, onDone }: { comp?: Competitive; id?: string; owner?: boolean; onDone?: () => void }) {
  ...
  return (
    <div className="card">
      <div className="th-comp-bar">
        <div className="th-comp-bar-l">
          {comp?.space ? <div className="th-comp-cap">The market — {comp.space}</div> : null}
          {!empty ? (
            <div className="th-comp-tabs" role="tablist">
              <button role="tab" aria-selected={view === "cards"} className={"th-comp-tab" + (view === "cards" ? " on" : "")} onClick={() => setView("cards")}>Detailed Cards</button>
              <button role="tab" aria-selected={view === "table"} className={"th-comp-tab" + (view === "table" ? " on" : "")} onClick={() => setView("table")}>Tabular View</button>
            </div>
          ) : null}
        </div>
        {owner && id ? (
