...
Default OFF (Rule 20): enable with EIGEN_VIDEO_ENABLED=true. Generation spends OpenAI/
Anthropic credits (storyboard LLM + TTS), so it is opt-in and never on the answer path.

Config (env):
  EIGEN_VIDEO_ENABLED   "true" to mount the router (default off).
  EIGEN_VIDEO_DIR       the in-repo video app dir (default <repo>/apps/video).
  EIGEN_VIDEO_TSX       tsx binary (default <video_dir>/node_modules/.bin/tsx).
...
def video_enabled() -> bool:
    return os.environ.get("EIGEN_VIDEO_ENABLED", "").lower() in ("1", "true", "yes")


def _video_dir() -> Path:
    default = Path(__file__).resolve().parent.parent / "video"   # <repo>/apps/video
    return Path(os.environ.get("EIGEN_VIDEO_DIR", str(default)))
...
def _tsx() -> str:
    return os.environ.get("EIGEN_VIDEO_TSX", str(_video_dir() / "node_modules" / ".bin" / "tsx"))
...
async def _run_bridge(job_id: str, payload: dict, session_id: str | None = None,
    ...
    try:
        ...
        env = {**os.environ, "EIGEN_VIDEO_OUT_DIR": str(_out_dir())}
