...
Skipped unless EIGEN_CORPUS_DSN points at Postgres (mirrors the other integration
tests). Seeds a small GROUNDED graph under a UNIQUE tenant, then drives the HTTP surface
via TestClient:

  * flag OFF (EIGEN_CROSSVIEWS unset) → all four endpoints 404 (true no-op);
...
pytestmark = [
    ...
    pytest.mark.skipif(not DSN, reason="set EIGEN_CORPUS_DSN for crossviews endpoint tests"),
]
...
def test_flag_off_all_endpoints_404(monkeypatch) -> None:
    monkeypatch.delenv("EIGEN_CROSSVIEWS", raising=False)
...
def test_options_build_save_reopen(monkeypatch) -> None:
    monkeypatch.setenv("EIGEN_CROSSVIEWS", "1")
    monkeypatch.setenv("EIGEN_CORPUS_DSN", DSN)
