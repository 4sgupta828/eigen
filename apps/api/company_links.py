...
Known companies additionally carry `eigen_url` → the in-product grounded /entity page (the ◉).

Returns [{name, url, grounded: bool, search: bool, eigen_url?}] where `name` is the EXACT surface
...
async def detect_and_resolve_companies(service, store, *, answer: str, ui=None,
    ...
    for nm, homepage in picked:
        ...
        eigen_url = ("/entity/" + urllib.parse.quote(eid, safe="")) if eid else None
        grounded_page = None
        if eid:
            try:
                ...
                grounded_page = None
            grounded_page = grounded_page or eigen_url
