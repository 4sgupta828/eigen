...
def test_evidence_identity_flag_reads_env(monkeypatch) -> None:
    # Evidence Contract stage 1: the flag is wired from EIGEN_EVIDENCE_IDENTITY (default OFF).
    from api.app import evidence_identity_enabled
    monkeypatch.delenv("EIGEN_EVIDENCE_IDENTITY", raising=False)
    assert evidence_identity_enabled() is False
    monkeypatch.setenv("EIGEN_EVIDENCE_IDENTITY", "1")
    assert evidence_identity_enabled() is True
    monkeypatch.setenv("EIGEN_EVIDENCE_IDENTITY", "false")
    assert evidence_identity_enabled() is False


def test_claim_congruence_flag_reads_env(monkeypatch) -> None:
    # Evidence Contract stage 2: the flag is wired from EIGEN_CLAIM_CONGRUENCE (default OFF).
    from api.app import claim_congruence_enabled
    monkeypatch.delenv("EIGEN_CLAIM_CONGRUENCE", raising=False)
    assert claim_congruence_enabled() is False
    monkeypatch.setenv("EIGEN_CLAIM_CONGRUENCE", "1")
    assert claim_congruence_enabled() is True
    monkeypatch.setenv("EIGEN_CLAIM_CONGRUENCE", "false")
    assert claim_congruence_enabled() is False


def test_question_contract_mode_reads_env(monkeypatch) -> None:
    # Evidence Contract stage 3: EIGEN_QUESTION_CONTRACT is a MODE string (mirrors
    # EIGEN_GRAPH_EXPAND): "" off (default), "shadow", "steer"; anything else → off.
    from api.app import question_contract_mode
    monkeypatch.delenv("EIGEN_QUESTION_CONTRACT", raising=False)
    assert question_contract_mode() == ""
    monkeypatch.setenv("EIGEN_QUESTION_CONTRACT", "shadow")
    assert question_contract_mode() == "shadow"
    monkeypatch.setenv("EIGEN_QUESTION_CONTRACT", "STEER")
    assert question_contract_mode() == "steer"
    monkeypatch.setenv("EIGEN_QUESTION_CONTRACT", "1")     # bare truthy is NOT a mode
    assert question_contract_mode() == ""


def test_answer_mode_routing_flag_reads_env(monkeypatch) -> None:
    # Evidence Contract stage 4: the flag is wired from EIGEN_ANSWER_MODE_ROUTING (default OFF).
    from api.app import answer_mode_routing_enabled
    monkeypatch.delenv("EIGEN_ANSWER_MODE_ROUTING", raising=False)
    assert answer_mode_routing_enabled() is False
    monkeypatch.setenv("EIGEN_ANSWER_MODE_ROUTING", "1")
    assert answer_mode_routing_enabled() is True
    monkeypatch.setenv("EIGEN_ANSWER_MODE_ROUTING", "no")
    assert answer_mode_routing_enabled() is False
...
def test_intake_v2_flag_reads_env(monkeypatch) -> None:
    # Guided Intake v2: the flag is wired from EIGEN_INTAKE_V2 (default OFF → v1 byte-identical).
    from api.app import intake_v2_enabled
    monkeypatch.delenv("EIGEN_INTAKE_V2", raising=False)
    assert intake_v2_enabled() is False
    monkeypatch.setenv("EIGEN_INTAKE_V2", "1")
    assert intake_v2_enabled() is True
    monkeypatch.setenv("EIGEN_INTAKE_V2", "false")
    assert intake_v2_enabled() is False
...
def test_triage_wrap_up_forces_ready(monkeypatch) -> None:
    ...
    from eigen_kernel.research.triage import TriageTurn
    monkeypatch.setenv("EIGEN_TRIAGE", "1")
    monkeypatch.delenv("EIGEN_INTAKE_V2", raising=False)
    monkeypatch.delenv("EIGEN_CORPUS_DSN", raising=False)   # env-only flag resolution (no DB)
...
def test_triage_v2_uses_v2_prompt_schema_and_case_cap(monkeypatch) -> None:
    # Under EIGEN_INTAKE_V2: the v2 directive + TriageTurnV2 schema are selected, the new fields
    ...
    from eigen_kernel.research.triage import TriageTurnV2
    monkeypatch.setenv("EIGEN_TRIAGE", "1")
    monkeypatch.setenv("EIGEN_INTAKE_V2", "1")
    monkeypatch.delenv("EIGEN_CORPUS_DSN", raising=False)   # env-only flag resolution (no DB)
...
def test_panel_upgrade_flags_read_env(monkeypatch) -> None:
    ...
    # EIGEN_PANEL_DEDUP / EIGEN_PANEL_CONTRACT (both default OFF).
    from api.app import panel_contract_enabled, panel_dedup_enabled
    monkeypatch.delenv("EIGEN_PANEL_DEDUP", raising=False)
    monkeypatch.delenv("EIGEN_PANEL_CONTRACT", raising=False)
    assert panel_dedup_enabled() is False and panel_contract_enabled() is False
    monkeypatch.setenv("EIGEN_PANEL_DEDUP", "1")
    monkeypatch.setenv("EIGEN_PANEL_CONTRACT", "true")
    assert panel_dedup_enabled() is True and panel_contract_enabled() is True
    monkeypatch.setenv("EIGEN_PANEL_DEDUP", "no")
    monkeypatch.setenv("EIGEN_PANEL_CONTRACT", "0")
    assert panel_dedup_enabled() is False and panel_contract_enabled() is False
...
def test_panel_payload_carries_coverage_gaps(monkeypatch) -> None:
    ...
    from eigen_kernel.research.panel import PanelResult
    monkeypatch.setenv("EIGEN_ASK_PANEL", "1")
    monkeypatch.delenv("EIGEN_CORPUS_DSN", raising=False)   # env-only flags, no session store
    svc = _service()
...
# ---- ✦ term glossary (EIGEN_TERM_GLOSSARY) ----
...
def test_terms_endpoints_404_when_flag_off(monkeypatch) -> None:
    monkeypatch.delenv("EIGEN_TERM_GLOSSARY", raising=False)
    client = TestClient(create_app(_service()))
...
def test_terms_explain_returns_terms_and_config_echo(monkeypatch) -> None:
    from eigen_kernel.research.terms import TermExplanation, TermExplanations
    monkeypatch.setenv("EIGEN_TERM_GLOSSARY", "1")
    monkeypatch.delenv("EIGEN_CORPUS_DSN", raising=False)
...
def test_glossary_lookup_explains_fresh_term(monkeypatch) -> None:
    from eigen_kernel.research.terms import TermExplanation
    monkeypatch.setenv("EIGEN_TERM_GLOSSARY", "1")
    monkeypatch.delenv("EIGEN_CORPUS_DSN", raising=False)
...
# ---- ◫ add-visuals (EIGEN_VISUAL_AUGMENT) ----

def _visuals_service(parsed):
    from eigen_kernel.research.visuals import VisualSet
    class _LLM2:
        async def complete(self, *, system, messages, response_format, max_tokens=2048, temperature=None):
...
def test_visuals_404_when_flag_off(monkeypatch) -> None:
    monkeypatch.delenv("EIGEN_VISUAL_AUGMENT", raising=False)
    client = TestClient(create_app(_service()))
...
def test_visuals_augment_returns_grounded_flow(monkeypatch) -> None:
    from eigen_kernel.research.visuals import Visual, VNode, VEdge, VisualSet
    monkeypatch.setenv("EIGEN_VISUAL_AUGMENT", "1")
    monkeypatch.delenv("EIGEN_CORPUS_DSN", raising=False)
