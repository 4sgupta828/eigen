...
- **Europe PMC full-text is opt-in** via `EIGEN_EPMC_FULLTEXT` (default off,
...
The flow (all flag-gated behind `EIGEN_GAP_HEALING`, default off):
...
> `noesis_corpus_gap_queue` (ingest jobs) all share `EIGEN_CORPUS_DSN`. Each
