...
- **`MEDICAL_CLINICAL_SYNTHESIS_FORMAT`** (`answer_format.py:69-123`) — an A/B
  variant behind `EIGEN_CLINICAL_SYNTHESIS` (default off). It uses the **same
...
Both are gated behind `EIGEN_STRUCTURED_ANSWERS`; when off, the kernel's plain
...
Both are opt-in: gap behind `EIGEN_GAP_HEALING`, suggest behind
`EIGEN_CONVERSATION` (`app.py:218-219`).
