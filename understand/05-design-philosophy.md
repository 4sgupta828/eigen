...
inventory in [the API layer](README.md): `EIGEN_STRUCTURED_ANSWERS`,
`EIGEN_CLINICAL_SYNTHESIS`, `EIGEN_VISION`, `EIGEN_GAP_HEALING`,
`EIGEN_STREAM`, `EIGEN_COUNTRY_SCOPE`, `EIGEN_CONVERSATION`,
`EIGEN_CLAIMS_FIRST`, `EIGEN_EVIDENCE_SELECT` (all in `app.py:29-93`, `225-230`).
...
attempted under pytest without an explicit `EIGEN_ALLOW_LIVE=1` opt-in
