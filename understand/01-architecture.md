...
`load_active_vertical()`): it looks up the `eigen.verticals` entry-point group
and picks the one named by the `EIGEN_ACTIVE_VERTICAL` environment variable
...
them offline for free. The mode is chosen by `EIGEN_PROVIDER_MODE`
...
if a `EIGEN_CORPUS_DSN` is set, create a `PostgresRetrievalSource` under the
