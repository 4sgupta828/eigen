...
## Part A — Evidence-fitness v1 (flag `EIGEN_EVIDENCE_FITNESS`, default OFF)
...
In `_rank_claims_by_relevance` (`react.py:279`), when `EIGEN_EVIDENCE_FITNESS` is on, combine the
...
2. **Part A (evidence-fitness v1)** → flip `EIGEN_EVIDENCE_FITNESS` ON in the eval.
