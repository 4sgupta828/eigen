# Eigen Claim-Graph — Design Improvement Spec
...
`eigen_vertical_tech` (`claim_predicates.py`, `diligence_compose.py`, `population_compose.py`).
...
1. Imports `eigen_vertical_tech.claim_predicates` in `ensure_schema` (`claimgraph.py:270`)
...
- `PeopleStore` — `eigen_entity` carries `vertical`/`kind` columns; labels caller-supplied.
...
| **Vertical move** | Physically relocate store to `packages/kernel/eigen_kernel/claimgraph/` | med | when vertical #2 lands |
