# Eigen — Path to a SOTA VC-Diligence Platform: Detailed Improvement Plan
...
Eigen's moat is **not** beating ChatGPT on freeform answers — it's **grounded, current, STRUCTURED
...
- Two causes: (a) the **Brave per-leg hydration** (now reverted behind `EIGEN_WEB_HYDRATE`) added an Exa
