# Eigen — Grounded Reasoning & Idea Generation (full spec)
...
leap was made. Eigen's defensible edge is **auditable grounded reasoning**: it reasons and
...
**The proof:** on reasoning/brainstorm questions, an independent judge scores Eigen (with this
feature) vs raw GPT. Eigen should win on **groundedness** and **low arbitrary-leap rate** while
...
- New kernel module `packages/kernel/eigen_kernel/research/reason.py` — domain-free. Pure function
...
- `EIGEN_DERIVE` — master switch for the reasoning gate + `derivations` output.
- `EIGEN_DERIVE_IDEAS` — also generate `opportunity`/brainstorm claims (rides EIGEN_DERIVE).
- OFF path is byte-identical to today.

## 8. The yardstick (fast, static — no waiting on the future)

`packages/kernel/eigen_kernel/eval/reason_scoring.py` + a fixture set of reasoning cases where we KNOW
which leaps are valid vs arbitrary. Each case: `{findings, valid_conclusions, arbitrary_conclusions}`.
Score (a vector):
- **soundness (precision):** of the leaps Eigen emits as `inference`, how many are actually valid?
  MUST be ≥ 0.95 (arbitrary leaps must never ship as confident inference).
- **coverage (recall):** of the known-valid leaps, how many did Eigen derive? (> 0; higher = better.)
...
- **Eigen** answer with `EIGEN_DERIVE(_IDEAS)` on (grounded + gated + labeled derivations);
...
1. **Groundedness** — can each substantive claim be traced to evidence? (Eigen should win big.)
2. **Arbitrary-leap rate** — fraction of claims that are unsupported jumps. (Eigen should be lower.)
3. **Idea usefulness** — are the generated ideas non-obvious and relevant? (should be ≈ parity.)
4. **Calibrated honesty** — does it distinguish fact from inference from speculation? (Eigen wins.)
Success = Eigen ≥ parity on idea usefulness AND clearly better on grounding + arbitrary-rate +
...
Eigen's Ask-Panel engine exists but the tech vertical set NO `panel_specialists` (empty tuple). The 5
...
3. **Wire it** (§6) behind `EIGEN_DERIVE`; run the yardstick live; tune until soundness ≥ 0.95.
4. **Prove vs GPT** (§9) — the head-to-head.
5. **Brainstorm mode** (§5) `EIGEN_DERIVE_IDEAS`.
