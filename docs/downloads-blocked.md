...
| 8 | **Reddit** | Broader-community SENTIMENT signal (r/MachineLearning, r/LocalLLaMA, r/startups, r/venturecapital…) — `sentiment_signal`, never controlling. | **CONNECTOR BUILT** (`connectors/reddit.py`, OAuth app-only). Keyless `.json` is dead — Reddit serves the HTML app shell to datacenter IPs (confirmed). INERT without creds. | Register a free "script" app at reddit.com/prefs/apps, set **`EIGEN_REDDIT_CLIENT_ID`** + **`EIGEN_REDDIT_CLIENT_SECRET`** on eigen-api, then run the `reddit` tranche. | 🟡 |
| 9 | **UK Companies House** | Non-US filings + company OFFICERS (the SEC-only geography gap + the team/execution lens) — `primary_filing`. | **CONNECTOR BUILT** (`connectors/companies_house.py`, HTTP Basic, inert without key). | Register a free key at developer.company-information.service.gov.uk, set **`EIGEN_COMPANIES_HOUSE_KEY`** on eigen-api, then run the `companies_house` tranche. | 🟡 |
| 10 | **USPTO patents** (PatentsView replacement) | IP-moat / prior-art evidence — `primary_filing` (granted) / `technical_signal` (application). Replaces the DORMANT patentsview (row 1). | **CONNECTOR BUILT** (`connectors/uspto.py`, Open Data Portal, `X-API-KEY`, reuses `patent_doc.py`, inert without key). NOTE: the ODP search endpoint/field-names are a marked `_normalize` point — CONFIRM against data.uspto.gov once a key exists (their docs are JS-rendered). | Register a key at data.uspto.gov, set **`EIGEN_USPTO_KEY`** on eigen-api, confirm `SEARCH_URL`/`_normalize`, then run the `uspto` tranche. | 🟡 |
