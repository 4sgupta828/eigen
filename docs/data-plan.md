# Eigen — Data-Download Plan (panel-reviewed) + Decoupling Plan
...
3. **Add retry/backoff + a rate limiter** to the connector HTTP (`connectors/_http.py`): honor SEC
   ...
   not a dead job. Set a **real** contact User-Agent (not `@eigen.example`).
...
### Rebrand (no "noesis" anywhere; fixes the kernel=eigen_ / apps=noesis_ split-brain)
- Rename ALL `apps/api` DDL + queries `noesis_*` → `eigen_*` **per module atomically**:
  ...
  the app recreates eigen_-named tables (loses only demo session/queue history — fine).
- Fix `backup.py:22-25` table list to the real `eigen_*` names (currently half-stale/broken).
- Rename the `x-noesis-token` header (app.py params + every `apps/web/*.html` fetch header) in
  LOCKSTEP → `x-eigen-token`. Rename FE storage keys `noesis-*`→`eigen-*`, SVG `#noesis-logo` id,
  and the asset files `noesis-logo.png`/`noesis-mark.png` (+ all refs).
- App title `Noesis Research`→`Eigen`; fallback HTML; docstrings/comments; `deploy/README.md`;
  `.env.example` (`EIGEN_ACTIVE_VERTICAL=tech`, an eigen DSN).
