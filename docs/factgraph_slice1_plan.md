# Eigen north-star: grounded temporal claim-graph + decision engine — panel synthesis & Slice-1 build plan
...
- **Decision-frame scaffold** — `eigen_vertical_tech/reasoned.py` already classifies decision questions into
  coverage dimensions (likely_causes/key_decisions/…) as questions-not-conclusions, pre-retrieval.
- **Graph precedent** — kernel `graph/store.py` + `eigen_edge_evidence` (span-grounded, corpus-populated,
...
- **Kernel-generic claim table vs vertical-owned.** Gemini + code-subagent: kernel-generic (domain-free,
  ...
  mechanics* (a `eigen_claim`/`eigen_entity` claim table + span-gate — the shape is already proven domain-free
...
Build:
1. **Tables** (kernel-generic, domain-free): `eigen_entity` (typed, natural-key, alias sidecar),
   `eigen_claim` (subject/predicate/object/valid_from/valid_to/confidence), `eigen_claim_evidence`
   (document_id/block_id/quote/quote_hash/source_key/authority_tier/evidence_status),
   `eigen_claim_resolution` (winner + conflict set), `eigen_predicate` (registry: active/candidate),
   `eigen_extraction_run` (audit + cost).
