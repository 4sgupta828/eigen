# EIGEN_INTELLIGENCE_CORE (Phase 1) — adversarial hypothesis-driven retrieval + grounded synthesis
...
**Why:** eigen already has ~9 of the 15 "intelligence" principles (deep-synthesis = thesis/mechanism/
...
- NEW `packages/kernel/eigen_kernel/research/intelligence_draft.py`:
...
- Flag: `apps/api/app.py` `intelligence_core_enabled()` (EIGEN_INTELLIGENCE_CORE) → ResearchService →
...
- Port factra's `finding_grounding_gate` pattern (`/Users/sgupta/factra/app/research_system/services/
  agents/research_orchestrator/finding_grounding_gate.py`) into eigen as a post-compose Layer-1 prose
