# EIGEN_AUTHORITY_BASIS — base answers on authoritative sources, keep the denoiser's breadth
...
byte-identical); keep EIGEN_WEB_OPEN_DENOISE breadth — NEVER re-privilege the 38-domain whitelist.
...
- Today's ranking (`react.py::_rank_claims_by_relevance` ~498) = `cosine + 0.15*(rank/6)` — BOOST-ONLY,
  ...
  ⇒ Enabling EIGEN_EVIDENCE_FITNESS alone is a VERIFIED NO-OP. The fix must be UNCONDITIONAL.
...
- `apps/api/app.py`: `authority_basis_enabled()` (EIGEN_AUTHORITY_BASIS, default OFF, mirror
...
- `packages/kernel/eigen_kernel/runtime/research.py`: ResearchService field `authority_basis: bool=False`;
  pass into `run_react`.
- `packages/kernel/eigen_kernel/research/react.py`: `run_react` param `authority_basis: bool=False`.
...
- `packages/vertical_tech/eigen_vertical_tech/answer_format.py`: add `TECH_AUTHORITY_BASIS_DIRECTIVE`:
...
- `packages/kernel/eigen_kernel/research/web_quality.py`: extend the screen verdict schema with an
...
- `packages/vertical_tech/eigen_vertical_tech/web_quality.py`: extend WEB_QUALITY_PROMPT to ALSO emit the
