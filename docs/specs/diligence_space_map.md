# Eigen — VC-Diligence Space/Thesis Map (design spec)
...
liability, not an asset). This is the FIRST diligence artifact. See [[eigen-strategic-reframe]].
...
- CROSSVIEWS (flag EIGEN_CROSSVIEWS): grounded dynamic tables. `apps/api/crossviews.py::build_grid`,
...
- POPULATION MAP (flag EIGEN_STARTUP_POPULATION): `/research/population` →
...
- CLAIM-GRAPH (`claimgraph.py::ClaimGraphStore`, `EIGEN_CORPUS_DSN`): BITEMPORAL grounded store —
...
- DEEP-COMPANY READER (flag EIGEN_DEEP_COMPANY_READER): `deep_company.py::retrieve_deep_company` — resolves
...
- DISCOVERY (flag EIGEN_DISCOVERY): `/discover` → `aggregate_entities` — "which companies work on X" but
...
- CURRENCY: EIGEN_PULSE (source-level change ledger + demotion) + EIGEN_FRESHNESS_RANKING (answer-level
...
Flag: EIGEN_DILIGENCE_MAP (new), default off, OFF byte-identical. Credits: the coverage loop is the
