# EIGEN_WEB_OPEN_DENOISE — open Exa to the full web + multi-gate denoising funnel

## Contract (Rule 1)

**Behavior:** When `EIGEN_WEB_OPEN_DENOISE` is ON, the aux web (Exa) leg searches the FULL
...
Gemini + code-grounded subagent); generalizes the prod-proven `EIGEN_WEB_ENTITY_OPEN` screen from
...
- Rule 20: `EIGEN_WEB_OPEN_DENOISE` default OFF; OFF path byte-identical (leg set, `_rerank`, and
...
`packages/kernel/eigen_kernel/research/web_quality.py`:
...
`packages/kernel/eigen_kernel/contract/dto.py`: add `web_denoise: bool = False` to `RetrievalRequest`
(next to `web_open`), default False → OFF byte-identical.

`packages/kernel/eigen_kernel/retrieval/web.py`:
...
`packages/kernel/eigen_kernel/runtime/research.py`: add `ResearchService.web_open_denoise: bool = False`;
pass `web_open_denoise=self.web_open_denoise` into `run_react` (next to `entity_open_web`).
`apps/api/app.py`: add `open_web_denoise_enabled()` (reads `EIGEN_WEB_OPEN_DENOISE`); pass
