# Eigen — Tech Knowledge Graph + Autonomous Crawler (v2, panel-converged)
...
See [[eigen-strategic-reframe]] (why), [[eigen-claim-graph]] (prior graph work — its DATA is discarded).

## 0. Thesis + the moat (sharpened)

Stop competing with ChatGPT on freeform answers (a race eigen loses). The moat is **auditable diligence
