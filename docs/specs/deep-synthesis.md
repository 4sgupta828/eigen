# EIGEN_DEEP_SYNTHESIS — deep, novel, grounded synthesis as the default answer

## Contract (Rule 1)
When `EIGEN_DEEP_SYNTHESIS` is ON, a non-lookup question's answer is a **synthesis-first grounded
...
- `manifest.py`: `deep_synthesis_on()` (EIGEN_DEEP_SYNTHESIS); expose the deep format + a flag the
...
- Only flip EIGEN_DEEP_SYNTHESIS ON in prod after the eval clears.
