...
- NEW `packages/kernel/eigen_kernel/providers/openai_client.py`: `class OpenAILLMClient` implementing the
  `LLMClient` protocol (`packages/kernel/eigen_kernel/providers/llm.py`): `async def complete(*, system,
  ...
  (`EIGEN_JUDGE_MODEL`, default a current OpenAI model, e.g. "gpt-4o" — match what openai_llm uses/expects).
...
- `apps/api/app.py::build_default_service`: construct `OpenAILLMClient()` when `OPENAI_API_KEY` is set and
  ...
  makes derive's validity judge cross-family. Gate the wiring behind a flag `EIGEN_CROSS_FAMILY_JUDGE`
...
- NEW `packages/kernel/eigen_kernel/research/refuter.py`: `async def refute_hypothesis(claim, judge_llm, *,
