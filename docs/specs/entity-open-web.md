# EIGEN_WEB_ENTITY_OPEN — entity-scoped, quality-screened open-web augmentation

## Contract (Rule 1)

**Behavior:** For a question the LLM judges to be diligence on a *specific named entity*
(company / product / project), and only when `EIGEN_WEB_ENTITY_OPEN` is ON, the research loop
...
- Rule 20: `EIGEN_WEB_ENTITY_OPEN` default OFF; OFF path **byte-identical** — no extra Exa call,
...
**Kernel — `packages/kernel/eigen_kernel/research/contract.py`:**
...
**Vertical — `packages/vertical_tech/eigen_vertical_tech/answer_contract.py`:**
...
**Manifest — `packages/vertical_tech/eigen_vertical_tech/manifest.py`:**
- Add `web_entity_open_on()` reading `EIGEN_WEB_ENTITY_OPEN` (default false), same style as other
...
**New file `packages/kernel/eigen_kernel/research/web_quality.py`:**
...
`packages/kernel/eigen_kernel/contract/manifest.py`, default None — domain-free).

**Tests:** fake-LLM keep/drop; empty hits → []; llm None / prompt None → []; exception → [].

### T3 — Open-web leg wiring in the ReAct loop

**`packages/kernel/eigen_kernel/research/react.py`:**
...
**`packages/kernel/eigen_kernel/runtime/research.py`:** add `entity_open_web: bool = False` and
...
**`apps/api/app.py`:** add `entity_open_web_enabled()` (reads `EIGEN_WEB_ENTITY_OPEN`, default
