# Eigen Reflection Pass — implementation spec

Flag: `EIGEN_REFLECTION` = `off` (default) | `shadow` | `steer`. OFF byte-identical (Rule 20).
...
confident-wrong intent before it acts, mirrors EIGEN_QUESTION_CONTRACT shadow). `steer`: all wiring live.
