# EIGEN_PARAMETRIC_LED — parametric-drafts-reasoning, retrieval-verifies-every-fact
...
facts confidently. eigen's differentiator is TRUST — so the model may LEAD, but every FACT it asserts
...
- `apps/api/app.py`: `parametric_led_enabled()` (EIGEN_PARAMETRIC_LED, default OFF); pass into
...
- New module `packages/kernel/eigen_kernel/research/prior_draft.py`:
