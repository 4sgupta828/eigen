#!/usr/bin/env python3
"""Head-to-head: Eigen's GROUNDED reasoning gate vs raw GPT reasoning, on adversarial cases.

Each case plants a TEMPTING-BUT-INVALID leap (a trap) among valid derivations. Ungrounded reasoning
tends to assert the trap fluently; Eigen's gate should REJECT it (never emit as inference) while still
deriving the valid steps. We run both and write a side-by-side report; an independent judge (Gemini,
different model family from both Claude-Eigen and GPT) then scores it.

Run: EIGEN_PROVIDER_MODE=live ANTHROPIC_API_KEY=... OPENAI_API_KEY=... \
...
sys.path.insert(0, os.path.join(os.path.dirname(__file__), "..", "packages", "vertical_tech"))

from eigen_kernel.research.reason import derive          # noqa: E402
from eigen_kernel.runtime.build import build_llm          # noqa: E402
...
CASES = [
    ...
    },
    {
        ...
        "trap": "(none — this case checks Eigen EMITS the valid transitive inference, not just suppresses traps.)",
    },
]
...
async def main():
    ...
    print("# Eigen grounded reasoning vs raw GPT — adversarial head-to-head\n")
    for i, case in enumerate(CASES, 1):
        ...
        eigen = await _eigen(case, llm)
        print("**EIGEN (gated, labeled derivations):**")
        if not eigen:
