...
Skipped unless EIGEN_TEST_PG_DSN points at a pgvector-enabled Postgres, so the
offline suite stays green without a DB. Mirrors the in-memory guarantees against
the real backend + checks ranking parity.

    EIGEN_TEST_PG_DSN=postgresql://strata:strata@localhost:5433/eigen_test \
...
from eigen_kernel.contract.dto import Locator, RetrievalRequest
...
DSN = os.environ.get("EIGEN_TEST_PG_DSN")
pytestmark = pytest.mark.skipif(not DSN, reason="set EIGEN_TEST_PG_DSN for pg integration")
