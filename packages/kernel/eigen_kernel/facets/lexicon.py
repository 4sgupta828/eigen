...
def find_spans(text: str, schema: FacetSchema, kind: str, *, aliases: dict | None = None,
    ...
    while i < len(toks):
        ...
        for n in range(min(max_ngram, len(toks) - i), 0, -1):
            ...
            if not keys:
                continue
            # One key reached twice — once from the schema's own vocabulary and once from an alias that
            # restates it — is not two readings. Left undeduped, "acquired" looked ambiguous with itself
            # and was demoted to a preference.
...
def claimed_by_filters(text: str, contract, *, filler: frozenset = frozenset(),
                      spans: list | None = None) -> str:
    """The words of `text` that NO filter accounts for — the semantic query that is actually left.

    `plan_from_spans` answers this for words the lexicon itself recognised. This answers it for the
    contract as a whole, after the compiler has run, and it is the half that matters for an open
    vocabulary: "companies backed by Amplify" compiles correctly to `investor=[amplify]`, and the
    lexicon cannot see it, because an investor slug is not a closed vocabulary anyone can enumerate.

    Leaving the whole brief in `text` is not harmless. The evaluator runs a semantic leg on it and fuses
    the result, so a query whose every word became a filter also ranks by similarity to those same
    words — and a company merely NAMED "Amplify" outranks the companies Amplify actually funded. What is
    returned here is the leftover; the caller decides whether to blank.
    """
    toks = [t[0] for t in _tokens(text)]
    if not toks:
        return ""
    claimed: set[str] = set(filler)
    # The words the reader actually TYPED for a value, which are not always the value. "India" becomes
    # `country=in` through an alias, and crediting only the canonical value leaves "india" looking like
    # unexplained prose — so a brief that was entirely filters keeps a semantic leg it does not need.
    for sp in (spans or []):
        claimed.update(_WORD.findall(str(getattr(sp, "text", "")).lower()))
    for section in ("must", "prefer", "avoid"):
        for key, vals in (getattr(contract, section, {}) or {}).items():
            claimed.update(_WORD.findall(str(key).lower()))
            if isinstance(vals, dict):
                continue                      # a numeric range names no word in the query
            for v in (vals if isinstance(vals, (list, tuple)) else [vals]):
                claimed.update(_WORD.findall(str(v).lower()))
    for key, vals in ((getattr(contract, "scope", {}) or {}).get("exclude") or {}).items():
        claimed.update(_WORD.findall(str(key).lower()))
        for v in (vals if isinstance(vals, (list, tuple)) else [vals]):
            claimed.update(_WORD.findall(str(v).lower()))
    return " ".join(t for t in toks if t not in claimed)


def may_blank_text(contract, residual: str, *, scope_keys: tuple = ()) -> bool:
    """May the caller drop the semantic text and let the filters BE the search?

    Two conditions, and both are load-bearing. Nothing may be left unexplained, or the part of the
    query the filters did not cover would simply be discarded. And something must actually narrow:
    blanking a contract whose only must is the scope — a country that is on every search — hands the
    evaluator an enumerate leg over the whole index and lets its cap pick which arbitrary rows the
    reader sees, which is worse than the diffuse embedding it replaced.
    """
    if (residual or "").strip():
        return False
    real = [k for k in (getattr(contract, "must", {}) or {}) if k not in scope_keys]
    return bool(real)
