...
def parse(raw: dict | None, schema: FacetSchema, kind: str, *, max_readings: int = 3) -> IntentCheck | None:
    ...
    for item in (raw.get("readings") or [])[: max_readings * 2]:
        ...
        if not reads_as_prose(r.text):
            # The filters were restated as a query. Drop the fake query, not the reading: the label and the
            # facet edits may still be good, and often are — "Firms that say they do pre-seed" is a real
            # distinction even when the model wrote `investor_type:seed_fund` in the wrong field.
            r.text = ""
        if r.must or r.prefer or r.text:
            out.readings.append(r)
        if len(out.readings) >= max_readings:
            break
    out.readings = distinct_readings(out.readings)
...
# A `text` that restates the contract in facet syntax — `still_deploying: ["yes_recent"] AND ai_infra` —
# is a filter wearing prose clothing. It cannot be a semantic query (nothing in the corpus is written that
# way), and it defeats the distinctness rule by making two chip-pairs look like two different searches.
_FACET_SYNTAX = re.compile(r"""(?x)
    [a-z_]{3,}\s*:\s*[\[\"']          # key: ["value"]  /  key: 'value'
  | \b[a-z_]{3,}:[a-z0-9_]              # key:value  — no space, which prose never does
  | \]\s*(and|or)\s*\[                 # ] AND [
  | ^\s*[a-z_]{3,}\s*=\s*\S             # key=value
""", re.I)


def reads_as_prose(text: str) -> bool:
    """Is this a semantic query a person would type, or the contract restated in facet syntax?"""
    t = (text or "").strip()
    if not t:
...
def distinct_readings(readings: list) -> list:
    """Drop readings that are the SAME reading with a different filter on top.

    A reading is a way of understanding the words; two of them must retrieve different things because they
    MEAN different things. Two options that run the same semantic query and differ only in which facet value
    they attach are one reading and a chip — and the reader already has every chip on the rail, with counts.

    Measured the first time this ran on a real model: "AI infra seed funds" and "AI infra venture funds",
    identical `text`, differing only by `investor_type`. Both were legal, both were useless, and no prompt
    wording is a reliable defence against a plausible-looking pair — so the check is structural. The
    semantic query is the identity of a reading; where two share one, only the first survives.
    """
    seen: set = set()
    out: list = []
    for r in readings or []:
        key = " ".join(str(getattr(r, "text", "") or "").lower().split())
