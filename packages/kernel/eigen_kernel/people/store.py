...
_DDL = """
CREATE TABLE IF NOT EXISTS eigen_entity (
...
CREATE INDEX IF NOT EXISTS ne_spec_geo ON eigen_entity (vertical, specialty, country, state);
CREATE INDEX IF NOT EXISTS ne_taxonomy ON eigen_entity (taxonomy);
CREATE INDEX IF NOT EXISTS ne_name ON eigen_entity (lower(name));
CREATE TABLE IF NOT EXISTS eigen_entity_metric (
...
CREATE INDEX IF NOT EXISTS nem_metric ON eigen_entity_metric (metric_key, period, value DESC);
CREATE TABLE IF NOT EXISTS eigen_entity_edge (
...
CREATE INDEX IF NOT EXISTS nee_a ON eigen_entity_edge (entity_a, relation);
CREATE INDEX IF NOT EXISTS nee_b ON eigen_entity_edge (entity_b, relation);
CREATE INDEX IF NOT EXISTS nee_ev ON eigen_entity_edge (relation, evidence_key);
CREATE TABLE IF NOT EXISTS eigen_entity_affiliation (
...
CREATE INDEX IF NOT EXISTS nea_key ON eigen_entity_affiliation (kind, affil_key);
CREATE TABLE IF NOT EXISTS eigen_entity_contact (
    entity_id    text NOT NULL,
...
class PeopleStore:
    ...
    async def set_status(self, entity_id: str, status: str) -> bool:
        ...
        async with pool.acquire() as conn:
            res = await conn.execute(
                "UPDATE eigen_entity SET status=$2, updated_at=now() WHERE entity_id=$1",
                entity_id, status)
        return res == "UPDATE 1"
    ...
    async def search(self, *, specialty: str = "", taxonomy: str = "", state: str = "",
        ...
        if sort_metric:
            ...
            join = (f"LEFT JOIN LATERAL (SELECT sum(value) AS mv FROM eigen_entity_metric m "
            ...
            join = (f"JOIN LATERAL (SELECT 1 FROM eigen_entity_metric m WHERE "
                    f"m.entity_id = e.entity_id AND m.metric_key = ${len(args)} LIMIT 1) mx ON true")
        args.append(min(max(limit, 1), 200))
        sql = (f"SELECT e.*{', mt.mv AS sort_value' if sort_metric else ''} FROM eigen_entity e "
    ...
    async def breakdown(self, *, specialty: str = "", taxonomy: str = "", state: str = "",
        ...
        async with pool.acquire() as conn:
            out["total"] = await conn.fetchval(
                f"SELECT count(*) FROM eigen_entity e WHERE {where}", *args)
            for col in ("specialty", "state", "city"):
                rows = await conn.fetch(
                    f"SELECT e.{col} AS v, count(*) AS c FROM eigen_entity e WHERE {where} "
    ...
    async def entity(self, entity_id: str) -> dict | None:
        ...
        pool = await self._get_pool()
        async with pool.acquire() as conn:
            e = await conn.fetchrow("SELECT * FROM eigen_entity WHERE entity_id=$1", entity_id)
            if not e:
                return None
            ms = await conn.fetch(
                "SELECT metric_key, metric_label, value, unit, period, detail, source, "
                "retrieved_at FROM eigen_entity_metric WHERE entity_id=$1 "
                "ORDER BY metric_key, period DESC", entity_id)
            cs = await conn.fetch(
                "SELECT kind, value, source, published_by_subject FROM eigen_entity_contact "
        ...
        async with pool.acquire() as conn:
            afs = await conn.fetch(
                "SELECT kind, affil_key, name, source FROM eigen_entity_affiliation "
    ...
    async def affiliations_for(self, entity_ids: list[str]) -> dict[str, list[dict]]:
        ...
        async with pool.acquire() as conn:
            rows = await conn.fetch(
                "SELECT entity_id, kind, affil_key, name FROM eigen_entity_affiliation "
    ...
    async def add_edges(self, edges: list[dict]) -> int:
        ...
        if rows:
            async with pool.acquire() as conn:
                await conn.executemany(
                    """INSERT INTO eigen_entity_edge (entity_a, entity_b, relation,
    ...
    async def derive_shared_key_edges(self, *, relation: str, metric_key: str,
        ...
        async with pool.acquire() as conn:
            res = await conn.execute(
                f"""INSERT INTO eigen_entity_edge (entity_a, entity_b, relation,
                      evidence_key, period, source)
                    SELECT LEAST(m1.entity_id, m2.entity_id), GREATEST(m1.entity_id, m2.entity_id),
                           $1, m1.detail, m1.period, $2
                    FROM eigen_entity_metric m1
                    JOIN eigen_entity_metric m2
                      ON m1.metric_key = m2.metric_key AND m1.detail = m2.detail
                     AND m1.period = m2.period AND m1.entity_id < m2.entity_id
                    JOIN eigen_entity e1 ON e1.entity_id = m1.entity_id AND e1.status='active'
                    JOIN eigen_entity e2 ON e2.entity_id = m2.entity_id AND e2.status='active'
                ...
                f"""SELECT c.peer, c.relation, c.evidence_key, c.source,
                    ...
                    FROM (
                      ...
                      SELECT a2.entity_id, 'same_' || replace(a1.kind, ' ', '_'),
                        ...
                        FROM eigen_entity_affiliation a1
                        JOIN eigen_entity_affiliation a2
                          ON a2.kind = a1.kind AND a2.affil_key = a1.affil_key
                         AND a2.entity_id != $1
                       WHERE a1.entity_id = $1
                    ) c
                    JOIN eigen_entity e ON e.entity_id = c.peer AND e.status = 'active'
                    WHERE true {preds}
    ...
    async def stats(self) -> dict:
        ...
        async with pool.acquire() as conn:
            n = await conn.fetchrow(
                ...
                "count(*) FILTER (WHERE status != 'active') AS non_active FROM eigen_entity")
            m = await conn.fetchval("SELECT count(*) FROM eigen_entity_metric")
            c = await conn.fetchval("SELECT count(*) FROM eigen_entity_contact")
    ...
    async def facet_values(self) -> dict:
        ...
        pool = await self._get_pool()
        async with pool.acquire() as conn:
            sp = await conn.fetch("SELECT DISTINCT specialty FROM eigen_entity "
                                  "WHERE specialty != '' ORDER BY specialty")
            st = await conn.fetch("SELECT DISTINCT state FROM eigen_entity "
                                  "WHERE state != '' ORDER BY state")
            co = await conn.fetch("SELECT DISTINCT country FROM eigen_entity "
                                  "WHERE country != '' ORDER BY country")
            mk = await conn.fetch("SELECT DISTINCT metric_key, metric_label "
                                  "FROM eigen_entity_metric ORDER BY metric_key")
