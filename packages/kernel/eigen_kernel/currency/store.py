...
- `eigen_change_event` is the SOURCE OF TRUTH. Block facet stamps (`superseded_by`,
...
_DDL = """
CREATE TABLE IF NOT EXISTS eigen_change_event (
    id               text PRIMARY KEY,
...
CREATE INDEX IF NOT EXISTS eigen_change_event_status ON eigen_change_event (status, created_at DESC);
CREATE TABLE IF NOT EXISTS eigen_watch (
...
CREATE TABLE IF NOT EXISTS eigen_watch_seen (
    user_id  text NOT NULL,
    event_id text NOT NULL,
    seen_at  timestamptz NOT NULL DEFAULT now(),
    PRIMARY KEY (user_id, event_id)
);
CREATE TABLE IF NOT EXISTS eigen_pulse_state (
    key        text PRIMARY KEY,          -- e.g. retraction_scan | detect_scan | last_retraction_sweep
    value      jsonb NOT NULL DEFAULT '{}'::jsonb,
    updated_at timestamptz NOT NULL DEFAULT now()
);
CREATE TABLE IF NOT EXISTS eigen_topic (
...
ALTER TABLE eigen_topic ADD COLUMN IF NOT EXISTS kind text NOT NULL DEFAULT 'condition';
"""
...
class CurrencyStore:
    ...
    async def record(self, *, relation: str, old_document_id: str = "", new_document_id: str = "",
        ...
        async with pool.acquire() as conn:
            await conn.execute(
                """INSERT INTO eigen_change_event
                   ...
                   ON CONFLICT (id) DO UPDATE SET
                     ...
                     subjects = CASE WHEN eigen_change_event.subjects = '[]'::jsonb
                                     THEN EXCLUDED.subjects ELSE eigen_change_event.subjects END,
                     updated_at = now()""",
    ...
    async def set_status(self, event_id: str, status: str) -> bool:
        ...
        async with pool.acquire() as conn:
            row = await conn.fetchrow(
                "UPDATE eigen_change_event SET status=$2, updated_at=now() WHERE id=$1 "
    ...
    async def apply_stamps(self) -> int:
        ...
        async with pool.acquire() as conn:
            rows = await conn.fetch(
                "SELECT relation, old_document_id, new_document_id FROM eigen_change_event "
    ...
    async def list_topics(self, *, limit: int = 500, kind: str | None = None) -> list[str]:
        ...
        async with pool.acquire() as conn:
            rows = await conn.fetch(
                f"SELECT label FROM eigen_topic {where} ORDER BY created_at LIMIT $1", *args)
        ...
        async with pool.acquire() as conn:
            for lb in labels or []:
                ...
                if not lb:
                    continue
                row = await conn.fetchrow(
                    """INSERT INTO eigen_topic (label, norm, source, kind) VALUES ($1,$2,$3,$4)
    ...
    async def add_watch(self, *, user_id: str, topic: str, source: str = "manual") -> None:
        ...
        async with pool.acquire() as conn:
            await conn.execute(
                "INSERT INTO eigen_watch (user_id, topic, source) VALUES ($1,$2,$3) "
                "ON CONFLICT DO NOTHING", user_id, t, source)

    async def remove_watch(self, *, user_id: str, topic: str) -> None:
        pool = await self._get_pool()
        async with pool.acquire() as conn:
            await conn.execute("DELETE FROM eigen_watch WHERE user_id=$1 AND topic=$2",
    ...
    async def list_watches(self, *, user_id: str) -> list[dict]:
        ...
        async with pool.acquire() as conn:
            rows = await conn.fetch(
                "SELECT topic, source, created_at FROM eigen_watch WHERE user_id=$1 "
    ...
    async def inbox(self, *, user_id: str, limit: int = 50) -> list[dict]:
        ...
        async with pool.acquire() as conn:
            seen_rows = await conn.fetch(
                "SELECT event_id FROM eigen_watch_seen WHERE user_id=$1", user_id)
    ...
    async def set_state(self, key: str, value: dict) -> None:
        ...
        async with pool.acquire() as conn:
            await conn.execute(
                """INSERT INTO eigen_pulse_state (key, value) VALUES ($1, $2::jsonb)
    ...
    async def get_state(self, key: str) -> dict | None:
        pool = await self._get_pool()
        async with pool.acquire() as conn:
            row = await conn.fetchrow("SELECT value FROM eigen_pulse_state WHERE key=$1", key)
        if row is None:
            return None
        v = row["value"]
    ...
    async def mark_seen(self, *, user_id: str, event_id: str) -> None:
        ...
        async with pool.acquire() as conn:
            await conn.execute(
                "INSERT INTO eigen_watch_seen (user_id, event_id) VALUES ($1,$2) "
    ...
    async def sweep_declared(self, lineage: list[dict]) -> dict:
        ...
        for rel in lineage or []:
            ...
            async with pool.acquire() as conn:
                row = await conn.fetchrow(
                    "SELECT status FROM eigen_change_event WHERE id=$1", eid)
