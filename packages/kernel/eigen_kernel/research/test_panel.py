...
from eigen_kernel.research.react import AgentStep, ClaimOut, ComposedAnswer, InterpretationItem, \
...
def test_plan_panel_selects_and_ignores_unknown():
    from eigen_kernel.research.panel import plan_panel, PanelPlan, SpecialistPick
...
def test_plan_panel_failsafe_on_error():
    from eigen_kernel.research.panel import plan_panel
    class _Bad:
...
def test_panel_streams_tagged_progress_events_in_phase_order():
    ...
    from eigen_kernel.research.panel import _TRACE_FORWARD
    src = _source()
...
def test_specialist_trace_is_throttled_to_meaningful_events(monkeypatch):
    ...
    import eigen_kernel.research.panel as panel_mod
...
def test_specialist_timeout_panel_proceeds(monkeypatch):
    """One lens hangs → it times out (EIGEN_PANEL_SPECIALIST_TIMEOUT) and the panel completes on
    the other lens's findings, with the timeout surfaced on the take and the done event."""
    import eigen_kernel.research.panel as panel_mod
    monkeypatch.setenv("EIGEN_PANEL_SPECIALIST_TIMEOUT", "1")
...
def test_panel_deadline_cancels_stragglers(monkeypatch):
    ...
    import eigen_kernel.research.panel as panel_mod
    monkeypatch.setenv("EIGEN_PANEL_DEADLINE", "1")
    monkeypatch.setenv("EIGEN_PANEL_SPECIALIST_TIMEOUT", "60")
    async def fake_run_react(*, on_event=None, **kw):
...
def test_panel_synthesis_findings_cap(monkeypatch):
    """Pooled findings beyond EIGEN_PANEL_SYNTH_CLAIMS are trimmed before synthesis — the panel's
    claims list IS the capped list, so the synthesis [n] refs stay consistent."""
    import eigen_kernel.research.panel as panel_mod
    monkeypatch.setenv("EIGEN_PANEL_SYNTH_CLAIMS", "4")
    async def fake_run_react(*, on_event=None, **kw):
