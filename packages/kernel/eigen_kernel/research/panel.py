...
from eigen_kernel.research.atoms import identity_tag
from eigen_kernel.research.budget import BudgetState
from eigen_kernel.research.provenance import normalize
from eigen_kernel.research.react import (
...
async def plan_panel(*, question, roster, llm, selection_prompt=None) -> list[dict]:
    ...
    import os as _os
    _cap = max(2, int(_os.environ.get("EIGEN_PANEL_MAX", "6")))
    if len(out) > _cap:
        _log.info("panel plan trimmed %d→%d (EIGEN_PANEL_MAX): dropped %s",
...
import os as _os
# All convened specialists run in parallel by default (the roster cap EIGEN_PANEL_MAX=6 bounds
# fan-out; the retrieval pool is sized to match). Was hardcoded 3 — users watched lenses queue.
_PANEL_CONCURRENCY = max(1, int(_os.environ.get("EIGEN_PANEL_CONCURRENCY", "6")))
...
# ---- Shared panel contract (flag EIGEN_PANEL_CONTRACT) ------------------------------------------
...
async def run_panel(*, question, specialists, llm, embedder, make_retrievers, tenant_id,
    ...
    Latency guardrails (env-overridable): EIGEN_PANEL_SPECIALIST_TIMEOUT (default 240s) bounds each
    lens; EIGEN_PANEL_DEADLINE (default 420s) bounds ALL specialists — stragglers are cancelled and
    the panel synthesizes with whatever completed; EIGEN_PANEL_SYNTH_CLAIMS (default 40) caps the
    pooled findings fed to synthesis (slot-covering claims seated first); EIGEN_PANEL_SYNTH_ATTEMPTS
    ...
    spec_timeout = _env_int("EIGEN_PANEL_SPECIALIST_TIMEOUT", 240)   # seconds per specialist run
    panel_deadline = _env_int("EIGEN_PANEL_DEADLINE", 420)           # seconds for ALL specialists
    synth_claim_cap = _env_int("EIGEN_PANEL_SYNTH_CLAIMS", 40)       # pooled findings fed to synthesis
    synth_attempts = _env_int("EIGEN_PANEL_SYNTH_ATTEMPTS", 2)       # synthesis compose retries
    ...
    if synth_claim_cap > 0 and len(pooled) > synth_claim_cap:
        ...
        for _i in range(len(pooled)):
            if len(_keep) >= synth_claim_cap:
                break
            _keep.add(_i)
        _log.info("panel synthesis findings capped %d→%d (EIGEN_PANEL_SYNTH_CLAIMS)",
...
def _pooled_tokens(verified) -> set:
    from eigen_kernel.research.react import extract_hard_tokens
