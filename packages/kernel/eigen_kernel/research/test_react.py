...
from eigen_kernel.research.react import AgentStep, ClaimOut, run_react
    ...
    class _AlwaysSearch:
        ...
        async def complete(self, *, system, messages, response_format, max_tokens=2048, temperature=None):
            if "VERIFIED FINDINGS" in messages[0]["content"]:      # compose step honors its contract
                from eigen_kernel.research.react import ComposedAnswer
...
def test_compose_retries_transient_failure(monkeypatch):
    ...
    import eigen_kernel.research.react as react
...
def test_rank_claims_by_relevance_keeps_the_most_relevant():
    ...
    from eigen_kernel.research.react import _rank_claims_by_relevance, VerifiedClaim
    class _FE:
...
def test_zero_evidence_answer_converts_to_search():
    ...
    from eigen_kernel.research.react import AgentStep, ClaimOut, ComposedAnswer
    src = _source()
    calls = {"n": 0}
    class _LLM:
        async def complete(self, *, system, messages, response_format, max_tokens=2048, temperature=None):
...
def test_reasoning_conclusion_repaired_when_guard_blanks_it(monkeypatch):
    ...
    import eigen_kernel.research.react as react
    monkeypatch.setattr(react, "_COMPOSE_BACKOFF_S", 0)
    from eigen_kernel.research.react import (ComposedAnswer, ConfidenceRead, ConfidenceDim)
    src, make = _compose_setup()
    def compose_fn(call_n):
        return LLMResult(parsed=ComposedAnswer(
...
def test_prose_hard_token_scan_flags_unsupported_figure():
    from eigen_kernel.research.react import _unsupported_prose_tokens, VerifiedClaim
...
def test_strip_control_tags_removes_leaked_serialization():
    ...
    from eigen_kernel.research.react import strip_control_tags as s
