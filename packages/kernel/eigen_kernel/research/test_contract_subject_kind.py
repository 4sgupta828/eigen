"""T1 — Contract `subject_kind` (EIGEN_WEB_ENTITY_OPEN): opaque LLM subject judgment.
...
from eigen_kernel.research.contract import Contract, derive_contract
...
def _reload_manifest_module():
    ...
    # NOTE: eigen_vertical_tech/__init__ shadows the `manifest` attribute with a built instance, so
    # `import eigen_vertical_tech.manifest as m` yields the instance — fetch the real module module.
    import sys
    importlib.import_module("eigen_vertical_tech.manifest")
    return importlib.reload(sys.modules["eigen_vertical_tech.manifest"])


def test_manifest_prompt_off_is_original_identity(monkeypatch=None):
    from eigen_vertical_tech.answer_contract import TECH_CONTRACT_PROMPT
    os.environ.pop("EIGEN_WEB_ENTITY_OPEN", None)
...
def test_manifest_prompt_on_uses_entity_variant():
    ...
    os.environ["EIGEN_WEB_ENTITY_OPEN"] = "1"
    try:
        m = _reload_manifest_module()
