...
from eigen_kernel.contract.protocols import GatingPolicy, RetrievalSource
...
async def run_react(
    *,
    ...
    if collect_diagnostics:
        from eigen_kernel.providers.anthropic_llm import LLM_CALL_LOG as _LLM_CALL_LOG
        _call_log = []
    ...
    _wo_steer = (
        ...
        "RESPECT A RECENCY SCOPE: if the question scopes to RECENT / LATEST / NEWEST / a recent time window "
        "('last N years', 'this year', 'founded since <year>'), name the notable RECENT members that fit it "
        "(redirect from famous-ESTABLISHED to famous-RECENT — you know many recent prominent members), AND "
        "ALSO search for the CURRENT ones ('newest / latest / recently founded <year> <members>') so the "
        "very newest surface too. DROP members you know clearly PREDATE the window, however famous; but KEEP "
        "a plausibly-recent member even if a page doesn't state its founding date. Aim for a rich roster of "
        "RECENT members — never an empty table, and never a list of decade-old incumbents."
    ) if web_only else ""
    ...
    if (prior_draft is None and not result.verified_claims and not result.rejected_claims
        ...
        try:
            from eigen_kernel.research.fallback_grounder import ground_claimless
    ...
    if claim_congruence and result.verified_claims:
        ...
        if os.environ.get("OPENAI_API_KEY"):
            try:
                from eigen_kernel.research.claims_first import _ENTAIL_CHUNK, entail_claims
