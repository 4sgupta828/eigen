...
verticals via the `eigen.verticals` entry-point group and builds its registries
...
class VerticalManifest:
    ...
    # patient view (flag EIGEN_PATIENT_MODE). Composes from the SAME verified findings — same gates —
    ...
    # Optional ALTERNATIVE-modality directive + retrieval hint (flag EIGEN_MODALITY_MODE): the
    ...
    # Optional PANEL synthesis addenda (flag EIGEN_PANEL_CONTRACT — P1 decision synthesis).
    ...
    # EIGEN_QUESTION_CONTRACT): instructs ONE small LLM call to decide whether a question demands
