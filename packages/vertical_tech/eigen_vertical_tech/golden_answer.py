...
- RESPECT THE TIME / RECENCY SCOPE — but don't empty the table. If the question scopes to RECENT / LATEST \
/ NEWEST / "founded in the last N years" / "since <year>", DROP entities you KNOW are old — an established \
company founded 5-15+ years ago (GitHub 2008, Notion 2013, Figma 2012, Airtable 2012, Stripe 2010) is NOT \
a "recent startup" and must be left out even if prominent and well-documented. But KEEP the plausibly- \
recent members: a company you know or believe was founded within the window STAYS even if the evidence \
doesn't state its founding date — do NOT drop a genuinely-recent startup merely because the date isn't \
cited. Add a "Founded" column when useful. The goal is a RICH table of the RECENT members (a dozen-plus \
if the field supports it) — not empty placeholder rows, and not a list of decade-old companies. Empty \
"(startup A/B/C)" rows are a failure; so is a table of established incumbents.
