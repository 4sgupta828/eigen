...
The v1 API requires a FREE API key (X-Api-Key). Set EIGEN_PATENTSVIEW_KEY; without it the connector
...
class PatentsViewConnector:
    ...
    def __init__(self, *, patents: list[dict] | None = None, page_size: int = 25):
        ...
        self._key = os.environ.get("EIGEN_PATENTSVIEW_KEY", "")
    ...
    async def _search(self, query: str, limit: int) -> list[dict]:
        if not self._key:
            _log.warning("patentsview: EIGEN_PATENTSVIEW_KEY not set — skipping (no patents ingested)")
            return []
