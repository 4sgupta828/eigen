...
KEYED source (patentsview pattern): needs a FREE API key. Set EIGEN_COMPANIES_HOUSE_KEY; without it
...
class CompaniesHouseConnector:
    ...
    def __init__(self, *, companies: list[dict] | None = None, page_size: int = 20):
        ...
        self._key = os.environ.get("EIGEN_COMPANIES_HOUSE_KEY", "")
    ...
    async def _search(self, query: str, limit: int) -> list[dict]:
        if not self._key:
            _log.warning(
                "companies_house: EIGEN_COMPANIES_HOUSE_KEY not set — skipping (no companies ingested)")
            return []
