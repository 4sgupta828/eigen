...
Guidance for Claude Code when working in this repo: **Eigen**, a deep-tech research Q&A platform
(startups, technology, competition, market sentiment) for an investor / VC-diligence user. It is a
domain-agnostic **kernel** (`packages/kernel/eigen_kernel/`) plus a plug-in **tech vertical**
(`packages/vertical_tech/eigen_vertical_tech/`), served by a vertical-neutral app shell (`apps/`).

## What this is / layout

- `packages/kernel/eigen_kernel/` — DOMAIN-FREE platform: ingest → corpus (`Document→ParsedDoc→Block`
  → flat `rs_block` in Postgres: tsv + pgvector + jsonb facets) → hybrid retrieval → ReAct research
  loop → **verbatim span-check provenance gate** → synthesis. Names no domain noun.
- `packages/vertical_tech/eigen_vertical_tech/` — ALL tech vocabulary: connectors (SEC EDGAR, arXiv,
  …), `TechPersona`, the tech authority pyramid, answer format, extraction lenses, `SECTOR_PROFILES`
  (AI seeded), UI, eval gold — bundled into one `VerticalManifest` via `manifest.py:build_manifest()`.
- `apps/{api,web}` — the FastAPI app + web shell. One vertical active per deployment
  (`EIGEN_ACTIVE_VERTICAL=tech`), provider mode `EIGEN_PROVIDER_MODE` (replay/record/live).
...
- App locally: `EIGEN_ACTIVE_VERTICAL=tech PYTHONPATH=apps .venv/bin/python -c "from api.app import create_app; create_app()"`

## Prod / deploy (Railway)

- Prod: **https://eigen-api-production.up.railway.app** (Railway project `balanced-vibrancy`, service
  `eigen-api` + managed Postgres+pgvector). Image = `deploy/Dockerfile` (lean tech image).
- Deploy is `railway up` (uploads the current directory + builds the Dockerfile). **It is NOT wired to
  GitHub auto-deploy — a `git push` does not redeploy. Re-run `railway up -d -s eigen-api` after code
  changes.** Poll `railway service status` until `SUCCESS`.
- Ingest (prod-direct, see below): `POST /admin/corpus/ingest` with `{"jobs":[{"connector","query","limit"}]}`
  and header `X-Admin-Token: $EIGEN_ADMIN_TOKEN`; watch `GET /corpus/queue`.

## No coupling with noesis — code or data (STANDING DIRECTIVE)

Eigen is an INDEPENDENT product. It shares the kernel *design lineage* with noesis but must not
...
- **No `noesis` anywhere.** No `noesis`/`noesis_kernel`/`noesis_vertical_*` imports, package names,
  strings, table names, env vars, branding, or comments. The kernel package is `eigen_kernel`; env
  vars are `EIGEN_*`; the entry-point group is `eigen.verticals`.
- **No medical-vertical vocabulary or code.** No `condition`/`drug`/`trial`/`clinical`/`patient`/CAM/
  ...
  fields, generalize it or remove it — never leave dormant medical logic in eigen.
- **Separate data.** Eigen has its OWN R2 bucket and its OWN Postgres. Never read noesis's bucket,
  DSN, or `.env`. Secrets live in eigen's own Railway service.
...
Every feature is built kernel-first: `packages/kernel/eigen_kernel/` owns MECHANICS (retrieval,
